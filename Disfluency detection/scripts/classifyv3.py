...
def classify_audio_files(model, device, bundle, all_audio_files, true_labels):
    predictions = []
    ...
    for root, dirs, files in os.walk(audio_directory):
        for file in files:
            if file.lower().endswith('.wav'):
                ...
                true_labels.append(1 if "/Block/" in file_path else 0)

    return all_audio_files, true_labels

model = StutterNet(512)
model_path = "/content/drive/MyDrive/Ulima/Data/saves/DisfluencyNet_train_data_blk_quart.pt"
...
if len(true_labels) > 0:
    ...
    predictions = classify_audio_files(model, device, bundle, all_audio_files, true_labels)
else:
